...
class Order {
  ...
  Order._fromShopify(Map<String, dynamic> parsedJson) {
    try {
      ...
      if (parsedJson['successfulFulfillments'].length > 0 &&
          parsedJson['successfulFulfillments'][0]['trackingInfo'].length > 0) {
        statusUrl =
            parsedJson['successfulFulfillments'][0]['trackingInfo'][0]['url'];
      } else {
        statusUrl = parsedJson['statusUrl'];
      }
