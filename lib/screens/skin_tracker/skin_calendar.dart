...
import 'package:flutter/src/painting/basic_types.dart' as axi;
...
import '../weight_tracker/weight_tranformation_page.dart';
import 'skin_health_page.dart';
import 'skin_home_page.dart';

DateTime? dateofpic;
bool canGoBack = true;

class SkinCalendar extends StatefulWidget {
  ...
  @override
  _SkinCalendarState createState() => _SkinCalendarState();
}

class _SkinCalendarState extends State<SkinCalendar> {
...
// calculates week number of date
  ...
  List getYears() {
    ...
    List.generate(6, (index) {
      String a = (DateTime.now().year - 5 + index).toString();
      y.add(a);
    });
    return y;
  }
  ...
  PickedFile? image;
  Future getImage(progress, {front}) async {
    ...
    if (isCamera) {
      ...
      setState(() {
        if (image != null) {
          front ? tmpFileF = File(image!.path) : tmpFileT = File(image!.path);
        } else {
              ...
              if (progress.type == 'Skin') {
                if (frontPic != null || topPic != null) {
                  ...
                  unawaited(Navigator.pushReplacement(context,
                      MaterialPageRoute(builder: (context) => const SkinHomePage())));
                  canGoBack = true;
                }
              } else {
                if (frontPic != null && topPic != null) {
                ...
                if (frontPic != null && topPic != null) {
                  ...
                  unawaited(progress.type != 'Weight Loss'
                      ...
                      : Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                              builder: (context) => const WeightTransormatioPage())));
                }
              }
            });
      }
  ...
  String getBannerImg({front, type}) {
    if (type == 'Hair') {
      return front
  ...
  bool isThisMonth() {
    if (dropdownValue == DateTime.now().year.toString() &&
        num.parse(progress!.tracker!.months![showMonth].monthNumber!) - 1 ==
            _selectedMonthIndex) {
      return true;
    } else {
      return false;
...
//  If your calender starts from Monday
  ...
  Container detailsCard(ProgressTrackingVM progress, context) {
    return Container(
      key: detailscard,
                              ...
                              children: List.generate(
                                ...
                                return circularMarker(
                                    ...
                                    text: progress
                                            ...
                                            .conditions?[index]
                                            .shortName ??
                                        getConditionShortName(progress
                                            .skinData
                                            ?.images
                                            ?.months?[showMonth]
                                            .weeks?[_selectedWeekIndex]
  ...
  void initState() {
    ...
    for (int i = 0; i < progress!.tracker!.months!.length; i++) {
      if (int.parse(progress.tracker?.months![i].year.toString() ?? '') ==
              DateTime.now().year &&
          int.parse(
                  progress.tracker?.months![i].monthNumber.toString() ?? '') ==
              DateTime.now().month) {
        showMonth = i;
      }
    }
    _selectedMonthIndex = int.parse(
            progress?.tracker?.months![showMonth]?.monthNumber.toString() ??
                '0') -
        1;
    getPresentWeek();
                    ...
                    onChanged: (val) {
                      setState(
                        () {
                          ...
                          if (dropdownValue == DateTime.now().year.toString()) {
                            for (int i = 0;
                                i < progress!.tracker!.months!.length;
                                i++) {
                              if (int.parse(progress.tracker?.months![i].year
                                              .toString() ??
                                          '') ==
                                      DateTime.now().year &&
                                  int.parse(progress
                                              .tracker?.months![i].monthNumber
                                              .toString() ??
                                          '') ==
                                      DateTime.now().month) {
                                showMonth = i;
                            ...
                            _selectedMonthIndex = int.parse(pVm.tracker
                                        ?.months![showMonth].monthNumber ??
                                    '0') -
                                1;
                            getPresentWeek();
              ...
              Row(children: [
                ...
                SizedBox(
                  ...
                  child: ListView.builder(
                      ...
                      scrollDirection: axi.Axis.horizontal,
                      itemCount: 12,
                      ...
                      child: Padding(
                        ...
                        child: Container(
                            ...
                            child: Padding(
                              ...
                              child: Text(
                                pVm.type == 'Skin'
                                    ? 'Week ${pVm!.tracker!.months![showMonth]
                                            .weeks![index].weekNum!}'
                                    : pVm!
                                        .tracker!
                                        .months![showMonth]
                        ...
                        children: [
                          Text('${weeksUploaded()} weeks uploaded',
                              style: const ts.TextStyle(
                                  fontSize: 15, color: Colors.black)),
                          Text(
                              '${daysBetween(progress!.tracker!.startDate!,
                                          DateTime.now())} weeks missed',
                              style: const ts.TextStyle(
                                  fontSize: 15, color: Colors.black)),
                        ],
                      ),
                  ...
                  child: Padding(
                    ...
                    child: Text(
                      int.parse(dropdownValue ?? '') >
                              progress!.tracker!.startDate!.year
                          ? 'You can only upload the picture for current week'
                          : _selectedMonthIndex + 1 >
                                  progress!.tracker!.startDate!.month
                              ? 'You can only upload the picture for current week'
                              : 'Your Journey started from week ${weekOfMonth(progress!.tracker!.startDate) + 1} of ${DateFormat.yMMMM().format(progress!.tracker!.startDate!).toString()}',
                      textAlign: TextAlign.center,
                      style: const ts.TextStyle(
                          color: Colors.white,
                          fontSize: 16,
              ...
              child: Container(
                ...
                child: Center(
                  ...
                  child: Text(
                    show
                        ? _selectedWeekIndex <
                                    weekOfMonth(progress!.tracker!.startDate) &&
                                DateTime(int.parse(dropdownValue ?? ''),
                                            _selectedMonthIndex + 1)
                                        .isBefore(
                                            progress!.tracker!.startDate!) ==
                                    true
                            ? 'Your Journey started from week ${weekOfMonth(progress!.tracker!.startDate) + 1} of ${DateFormat.yMMMM().format(progress!.tracker!.startDate!).toString()}'
                            : _selectedMonthIndex + 1 != DateTime.now().month ||
                        ...
                        : _selectedMonthIndex + 1 >
                                progress!.tracker!.startDate!.month
                            ? 'You can only upload the picture for current week'
                            : 'Your Journey started from week ${weekOfMonth(progress?.tracker?.startDate) + 1} of ${DateFormat.yMMMM().format(progress?.tracker?.startDate ?? DateTime.now()).toString()}',
                    textAlign: TextAlign.center,
                    style: const ts.TextStyle(
                        color: Colors.white,
                        fontSize: 16,
  ...
  Container skinWidget(context, progress) {
    String doctorNote = progress.skinData.images.months[showMonth]
  ...
  String getMessage(var selectedYear, var selectedMonth, var selectedWeek) {
    if (getFirstDateOfSelectedWeek(selectedYear, selectedMonth, selectedWeek)
        .isBefore(getWeekStartDate(
            progress.skinData?.images?.startDate ?? DateTime.now()))) {
      return 'Your Journey started from week ${weekOfMonth(progress.skinData?.images?.startDate)} of ${DateFormat.yMMMM().format(progress.skinData?.images?.startDate ?? DateTime.now()).toString()}';
      ...
      return 'Uploading a picture is limited to current week only';
                ...
                selectedYear, selectedMonth, selectedWeek) ==
            getWeekStartDate(DateTime.now()) &&
        DateTime.now().month != selectedMonth) {
      return 'Uploading a picture is limited to current week only';
    } else if (getFirstDateOfSelectedWeek(
            selectedYear, selectedMonth, selectedWeek) ==
        getWeekStartDate(DateTime.now())) {
      return 'Your skin score will be updated once you upload a picture';
    } else {
      return 'Oops! It seems that you forgot to upload a picture for this week';
    }
  }

  dynamic getConditionShortName(String? id) {
    if (id == '62036d0119a68092b9b9e526') {
      return 'Acne';
    } else if (id == '62036d1419a68092b9b9e527') {
      return 'Wrinkles';
    } else if (id == '62036d3619a68092b9b9e528') {
      return 'Tan';
    } else if (id == '62036d4619a68092b9b9e529') {
      return 'Roughness';
    }
  }
